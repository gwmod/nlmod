...
def download_file(url, pathname, overwrite=False):
...
def add_buisdrainage(ds, pathname=None, cond_var="buisdrain_cond", depth_var="buisdrain_depth", cond_method="average", depth_method="mode"):
    if pathname is None:
    ...
    fname_c = download_file(url, pathname)
    ...
    # resample to model grid
    ds[depth_var] = structured_da_to_ds(buisdrain_d, ds, method=depth_method, nodata=nodata)
