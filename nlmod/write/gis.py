...
def dataarray_to_gdf(model_ds, data_variables, polygons=None,
                     dealing_with_time='mean'):
    """ Convert one or more DataArrays from a model dataset to a 
    Geodataframe.
...
def dataarray_to_shapefile(model_ds, data_variables, fname, polygons=None):
    ...
    """
    gdf = dataarray_to_gdf(model_ds, data_variables, polygons=polygons)
    gdf.to_file(fname)
...
def model_dataset_to_vector_file(model_ds,
    ...
    -------
    fnames : list of str
        filenames of exported shapefiles.
    ...
    for key, item in combine_dic.items():
        if set(item).issubset(da_names):
            gdf = dataarray_to_gdf(model_ds, item, polygons=polygons)
            if driver == 'GPKG':
    ...
    for da_name in da_names:
        gdf = dataarray_to_gdf(model_ds, (da_name,), polygons=polygons)
        if driver == 'GPKG':
