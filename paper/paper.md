...
It handles open-data, mainly from the Netherlands, efficiently via the popular Python packages `xarray` [@hoyer_xarray_2017] and `geopandas` [REFERENCE!!].
...
In general an nlmod scripts to build a groundwater flow model contain these steps:
...
Store all raster data in a single model dataset and vector data in one or multiple GeoDataFrames.
