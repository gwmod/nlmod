...
def test_time_options():
    ...
    _ = nlmod.dims.set_ds_time(ds, start='2000-1-1', time=pd.to_datetime(['2000-2-1', '2000-3-1']))
    ...
    _ = nlmod.dims.set_ds_time(ds, start=pd.Timestamp('2000-1-1'), 
                    time=pd.Timestamp('2000-2-1'))
...
def test_time_out_of_bounds():
    ...
    # start cf.datetime and time list of int
    _ = nlmod.dims.set_ds_time(ds, start=start_model, time=pd.to_datetime(['2000-2-1', '2000-3-1']))
    ...
    _ = nlmod.dims.set_ds_time(ds, start='1000-01-01', time=pd.to_datetime(['2000-2-1', '2000-3-1']))
    ...
    _ = nlmod.dims.set_ds_time(ds, start=np.datetime64('1000-01-01'), perlen=[10, 100, 24])

    # start numpy datetime and time list of timestamps
    _ = nlmod.dims.set_ds_time(ds, start=np.datetime64('1000-01-01'),
                    time=pd.to_datetime(['2000-2-1', '2000-3-1']))

    # start numpy datetime and time list of str
    with pytest.raises(TypeError):
        nlmod.dims.set_ds_time(ds, start=np.datetime64('1000-01-01'), time=['1000-2-1', '1000-3-1'])

    # start timestamp and perlen list of int
    _ = nlmod.dims.set_ds_time(ds, start=pd.Timestamp('1000-01-01'), perlen=[10, 100, 24])
